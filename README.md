...
A web page trades on those pools:

    https://seed.alpha.ecash.eu.com/dex/

The page makes a key in the browser, asks the faucet for pegged BTC, and
swaps. It also makes a token and a pool. `dex/src` holds its client, and the
same client drives the two operator scripts:

```sh
cd dex && npm ci && npm test && npm run build
RPC_URL=<rpc> KEYPAIR=<id.json> TOKENS_OUT=dist/tokens.json node dist/scripts/seed.mjs
RPC_URL=<rpc> node dist/scripts/trade.mjs
```

`scripts/regtest-dex.sh` proves the whole DEX on a running chain: the programs
are there, the client builds, and a token, a pool, and two trades work. The
integration workflow runs it on every pull request.
